...
func UploadGeneric(file io.Reader, fileSize int64, options ...UploadOption) (
	...
	_, err := client.PutObject(context.Background(), minioBucket, fileName, file, fileSize, _options)
	if err != nil {
		return "", err
	}

	return minioBucket + "/" + fileName, nil
}
...
func UploadGenericFromPath(filePath string, options ...UploadOption) (
	...
	_, err := client.FPutObject(context.Background(), minioBucket, fileName, filePath, _options)
	if err != nil {
		return "", err
	}
