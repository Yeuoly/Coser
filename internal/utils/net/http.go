...
func buildHttpRequest(method string, url string, options ...HttpOptions) (*http.Client, *http.Request, error) {
	...
	for _, option := range options {
		...
		case "payloadText":
			...
			req.ContentLength = int64(len(option.Value.(string)))
		case "payloadJson":
...
// DownloadFile is a function to download file from url
// return a reader and a header
func DownloadFile(url string, options ...HttpOptions) (io.ReadCloser, *ResponseHeader, error) {
